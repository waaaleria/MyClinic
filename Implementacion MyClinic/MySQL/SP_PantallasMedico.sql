...
DROP PROCEDURE IF EXISTS LISTAR_CITAS_MEDICO_TODAS;
...
-- Elimina un diagnostico de la cita
DELIMITER $
...
BEGIN
    ...
    SET d.activo = 0
    WHERE d.id_diagnostico = _id_diagnostico;
END$
...
DELIMITER $
CREATE PROCEDURE LISTAR_CITAS_MEDICO_TODAS(
    IN _id_medico INT
)
BEGIN
    SELECT 
    ...
    CONCAT(pa.hora_inicio, ' - ', pa.hora_fin) AS 'Horario',
    CONCAT(u.nombre, ' ', u.apellido) AS 'Paciente'
    FROM ProgramaAtencion pa
    INNER JOIN Medico m ON m.id_medico= pa.id_medico
    INNER JOIN CitaMedica cm ON cm.id_medico = m.id_medico
    INNER JOIN Paciente p ON p.id_paciente = cm.id_paciente
    INNER JOIN Usuario u ON u.id_usuario = p.id_usuario
    WHERE pa.id_medico = _id_medico
    ORDER BY pa.fecha DESC;
END$
